...
function localizeAnalysisValue(value: string, language: DocLanguage): string {
  if (language !== "en") return value;

  const map: Record<string, string> = {
    "Ferramenta CLI": "CLI Tool",
    "IA aplicada": "Applied AI",
    "Projeto de software": "Software Project",
    "Construir projeto com foco em portfolio e visibilidade profissional": "Build a project focused on portfolio and professional visibility",
    "Construir projeto com foco em portfólio e visibilidade profissional": "Build a project focused on portfolio and professional visibility",
    "Transformar aprendizado em projeto executavel": "Turn learning into an executable project"
    ,"Transformar aprendizado em projeto executável": "Turn learning into an executable project"
  };

  const normalized = value
    .replace("[alerta: possivel prompt injection detectado]", "[warning: possible prompt injection detected]")
    .replace("[alerta: possível prompt injection detectado]", "[warning: possible prompt injection detected]");
...
export function buildReadmeTemplate(
  ...
  const theme = localizeAnalysisValue(analysis.theme, language);
  const intent = localizeAnalysisValue(analysis.intent, language);
  const context = localizeAnalysisValue(analysis.context, language);
...
export function buildArchitectureTemplate(
  ...
  return [
    ...
    `${t(language, "Tema principal", "Main theme")}: ${theme}`,
    `${t(language, "Direção", "Direction")}: ${intent}`,
...
export function buildRoadmapTemplate(
  ...
  const summary = localizeAnalysisValue(analysis.summary, language);
...
export function buildIssuesSuggestionsTemplate(
  ...
  return [
    ...
    `${t(language, "Tema", "Theme")}: ${theme}`,
    `${t(language, "Objetivo", "Goal")}: ${intent}`,
    "",
